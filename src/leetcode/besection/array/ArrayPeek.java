 ...
 * 峰值元素是指其值大于左右相邻值的元素。
 ...
 思路：找峰值问题，典型的二分搜索，判断中间值是在山峰左边或右边。
 此题使用二分法可能遇到数组越界或死循环问题，有两个技巧可以避免这些问题，也是二分法中通用的技巧。
 1. 避免数组越界，如果是奇数个，mid正好是中间，如果是偶数个，mid是中间靠前一个，因此mid+1不会越界，mid - 1可能越界
 2. 死循环问题，注意mid移动时加减一的问题，是否需要排除mid节点，根据题意判断。
    此题中两种情况不同，当nums[mid] < nums[mid + 1]时mid一定不是山峰可以排除mid，但nums[mid] > nums[mid + 1]时mid可能是山峰，不能排除
