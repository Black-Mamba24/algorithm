 ...
 */
public class HouseRoberIII337 {
    public static void main(String[] args) {
        HouseRoberIII337 houseRoberIII337 = new HouseRoberIII337();
        TreeNode root = new TreeNode(3);
        root.left = new TreeNode(2);
        root.right = new TreeNode(3);
//        root.left.right = new TreeNode(3);
//        root.right.right = new TreeNode(1);
        System.out.println(houseRoberIII337.rob(root));
    }

    public int last = 0, now = 0;

    public int rob(TreeNode root) {
        if (root == null) {
            return 0;
        }
        int s0 = rob(root.left) + rob(root.right);
        int s1 = root.val;
        if (root.left != null) {
